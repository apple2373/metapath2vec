...
def parse_args():
	...
	parser.add_argument('--types',type=str,required=True,help='text file that has node types. each line is "node id <tab> node type"')
...
def main(args):
	...
	center_node_placeholder,context_node_placeholder,loss = build_model(BATCH_SIZE=args.batch,VOCAB_SIZE=len(dataset.nodeid2index),EMBED_SIZE=args.embedding_dim,NUM_SAMPLED=args.negative_samples,negative_sampler=None)
	optimizer = traning_op(loss,LEARNING_RATE=args.lr)
	train(center_node_placeholder,context_node_placeholder,loss,dataset,optimizer,NUM_EPOCHS=args.epochs,BATCH_SIZE=args.batch,LOG_DIRECTORY=args.log,LOG_INTERVAL=args.log_interval,MAX_KEEP_MODEL=args.max_keep_model)
